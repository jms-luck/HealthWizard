...
class MedicalAssistant:
    ...
    def analyze_medical_image(self, image_path: str) -> str:
        if not os.path.isfile(image_path):
            return "Error: Image file not found. Please provide a valid path to a medical image."
        try:
            print("Uploading and analyzing medical image...")
            file = genai.upload_file(path=image_path, display_name="Medical Image")
            # Verify the image is likely medical-related by checking its type
            img = Image.open(image_path)
            if img.format not in ["JPEG", "PNG", "BMP"]:
                return "Error: Unsupported image format. Please upload a JPEG, PNG, or BMP medical image."
            ...
            return f"{response.text}\n\nDisclaimer: This is not a definitive diagnosis. Consult a doctor for professional evaluation."
        except Exception as e:
            return f"Image analysis error: {str(e)}. Ensure the image is a valid medical image (e.g., rash, scar)."
    ...
    def get_voice_input(self) -> Optional[str]:
        ...
        try:
            ...
            return f"Voice input error: {str(e)}. Check your microphone and try again."
    ...
    def run(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                path = input("Image path (e.g., rash.jpg): ").strip()
                if path:
